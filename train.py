...
from evals.eval import do_knn_distance_eval, do_test_target_with_prototypes, do_test_target_with_prototypes_new
from models.data_helper import get_train_dataloader, get_val_dataloader
from models.resnet import ResNetFc
from optimizer.optimizer_helper import get_optim_and_scheduler
from utils.ckpt_utils import check_resume, load_ckpt, resume, save_ckpt
...
def get_args():
    ...
    parser.add_argument("--iterations", type=int, default=33000, help="Number of iterations")
    parser.add_argument("--step_after", type=int, default=33000, help="Step after")
    parser.add_argument("--warmup", type=int, default=500, help="Number of warmup iters")

    # learning objective
    parser.add_argument("--loss_function", type=str, default="H", choices=["BCE", "CE", "L2", "H"], help="Choose learning objective")
    ...
    parser.add_argument("--hidden_head_dim", type=int, default=256,
                        help="Dimensionality of the hidden layer in rel transformer head (if None, use a single FC instead of a MLP)")
    ...
    # hinge loss params
    parser.add_argument("--hinge_margin", type=float, default=0.01, help="Margin width for hinge loss")

    # focal loss params
    parser.add_argument("--focal_gamma", type=int, default=None, help="Gamma parameter for Focal Loss")
...
class Trainer:
    def __init__(self, args, device, folder_name):
        ...
        num_classes = 2 if self.args.loss_function == "CE" else 1

        # rel module
        from models.relational_transformer import RelationalTransformer

        self.cls_rel = RelationalTransformer(self.output_num, num_classes=num_classes, hidden_head_dim=args.hidden_head_dim,
                                             depth=args.transf_depth, num_heads=args.transf_n_heads)
    ...
    def _do_iteration(self, log=False):
        ...
        if self.args.loss_function == "L2":
            ...
            # p is used for loss computation only when using gamma (focal loss)
            with torch.no_grad() if self.args.focal_gamma is None else nullcontext():
                p = torch.sigmoid(self.args.sigmoid_compression * relation_logit)
            log_p_t = F.binary_cross_entropy_with_logits(
                relation_logit,
                relation_l.float(),
                reduction='none',
            )
            if self.args.focal_gamma is not None:
                p_t = (relation_l * p) + ((1 - relation_l) * (1 - p))
                log_p_t = log_p_t * torch.pow(1.0 - p_t, self.args.focal_gamma) 
            relation_loss = torch.mean(log_p_t)
            with torch.no_grad():
                # class prediction
                relation_pred = p > 0.5

        elif self.args.loss_function == "CE":
            log_p = F.log_softmax(relation_logit, dim=1)
            if self.args.focal_gamma is not None:
                p = F.softmax(relation_logit, dim=1)
                log_p = log_p * torch.pow(1.0 - p, self.args.focal_gamma)
            relation_loss = F.nll_loss(log_p, relation_l)
            with torch.no_grad():
                # class prediction
                _, relation_pred = relation_logit.max(dim=1)

        elif self.args.loss_function == "H":
            multipliers = torch.ones_like(relation_l)
            multipliers[relation_l == 0] = -1
            relation_loss = torch.clamp(self.args.hinge_margin - (multipliers * relation_logit), min=0).mean()
            with torch.no_grad():
                relation_pred = relation_logit > 0
